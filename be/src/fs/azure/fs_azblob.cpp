...
public:
    explicit AzBlobInputStream(BlobContainerClientPtr client, const AzBlobURI& uri)
...
StatusOr<int64_t> AzBlobInputStream::get_size() {
    if (_size == -1) {
        try {
            ...
            return azure_error_to_status(e.StatusCode,
                                         fmt::format("Blob object {} get properties error: {}", _uri.get_blob_uri(),
                                                     e.what()),
                                         _uri.get_blob_uri());
        } catch (const std::exception& e) {
            return Status::InternalError(fmt::format("Blob object {} get properties error: {}", _uri.get_blob_uri(),
                                                     e.what()));
        }
    }
    return _size;
...
public:
    explicit AzBlobOutputStream(BlobContainerClientPtr client, const AzBlobURI& uri)
            : _container_client(std::move(client)),
              _uri(uri),
              _blob_client(_container_client->GetBlockBlobClient(_uri.blob_name())),
              _multipart_upload(false) {}
...
Status AzBlobOutputStream::complete_multipart_upload() {
    try {
        ...
        return azure_error_to_status(e.StatusCode,
                                     fmt::format("Commit block list for blob object {} error: {}", _uri.get_blob_uri(),
                                                 e.what()),
                                     _uri.get_blob_uri());
    } catch (const std::exception& e) {
...
public:
    AzBlobClientFactory() { srand(time(NULL)); }
