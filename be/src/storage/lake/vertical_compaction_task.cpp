...
Status VerticalCompactionTask::execute(Progress* progress) {
    ...
    VLOG(3) << "Start vertical compaction. tablet id: " << _tablet->id()
                        << ", max_rows_per_segment: " << max_rows_per_segment
                        << ", column group size: " << column_group_size;

    ASSIGN_OR_RETURN(auto writer, _tablet->new_writer(kVertical, max_rows_per_segment));
    RETURN_IF_ERROR(writer->open());
    DeferOp defer([&]() { writer->close(); });

    auto source_mask_path = ExecEnv::GetInstance()->store_paths().begin()->path;
    auto mask_buffer = std::make_unique<RowSourceMaskBuffer>(_tablet->id(), source_mask_path);
...
Status VerticalCompactionTask::compact_column_group(bool is_key, int column_group_index, size_t column_group_size,
    ...
    VLOG(3) << "Compact column group. tablet id: " << _tablet->id() << ", column group: " << column_group_index
    ...
    while (true) {
        ...
        if (progress != nullptr) {
            progress->update((100 * column_group_index + 100 * reader.stats().raw_rows_read / _total_num_rows) / column_group_size);
