---
name: pr-fix-finder
description: 用户给出 StarRocks 的 crash 堆栈、报错信息、异常现象或 bug 描述，判断该问题是否已被历史 PR 修复，给出证据 PR 和修复版本。
---
...
默认假设 PR 检索服务已经启动，基础地址为 `http://172.21.188.190:8888`。如果用户或环境明确提供了其他地址，优先使用那个地址。
...
PR 的 `change_type`（BugFix/Refactor/Enhancement）、title、body 中的行为变更声明（如 "no behavior change"）仅作参考，**不能作为排除候选的依据**。原因：

- 作者可能未意识到 Refactor 同时修复了一个已有 bug
- `diff_keywords` 的 `symptom` 可能被生成为 "future risk" 或 "potential issue"，但实际变更已覆盖用户的真实 crash 路径
- 同一段代码变更既可以是重构也可以是修复，取决于观察角度

**判定时只看实际变更语义是否覆盖用户问题，不看 PR 怎么描述自己。**
...
### 需要 diff 验证的候选（`possible` 升级规则）

以下情况的 `possible` 候选**必须通过读代码 diff 验证**，不能仅凭 `diff_keywords` / `ai_summary` 排除：

1. **B 类强命中 + fix 涉及调用方式/路径变更**：候选的 symbols 命中用户 crash 路径中的核心符号（如 crash 栈中的类名/函数名），且 fix 描述了对该符号的调用方式、参数传递、分支路由等变更。即使 symptom 描述为 "future risk" 或 PR 类型为 Refactor，也必须读 diff 确认实际语义。
2. **fix 描述了收窄/限制某个优化或重写的适用范围**：如果用户的 crash 发生在某个优化路径中（如低基数优化、MV 重写、谓词下推），而候选 PR 的 fix 恰好收窄了该优化的适用条件，必须读 diff 确认收窄的条件是否覆盖用户的触发场景。
3. **候选的 symbols/files 与用户 crash 栈中 ≥ 2 个帧重合**：不能仅因为 symptom 不匹配就判 irrelevant。

读 diff 的方式优先 `git show <commit>` 或 `gh pr diff`，不要仅依赖 `/api/agent/pr` 返回的 body 和 `diff_keywords`。
...
### 隐式修复识别（Refactor-as-fix）

以下模式的 PR 可能被标注为 Refactor/Enhancement，但实际修复了 bug。不能仅因为 PR 类型不是 BugFix 就排除：

1. **路径收窄型**：PR 的 fix 将某个函数/表达式/优化从宽松路径改为带检查的严格路径（如 `mergeWithArray()` → `merge()`，后者比前者多了类型检查）。如果用户的 crash 恰好由宽松路径触发，该 PR 就是根因修复。
2. **分类修正型**：PR 将某类对象从一个处理集合移动到另一个（如将某函数从 `SET_A` 移到 `SET_B`，两个集合的处理逻辑不同）。如果用户的 crash 由原集合的处理逻辑触发，该 PR 就是修复。
3. **条件补全型**：PR 在已有的条件分支中增加了新的 case 或 guard（如 `if (isArray)` 分支原来缺失，PR 补上了）。
4. **调用顺序修正型**：PR 调整了初始化/准备阶段的调用顺序（如先 init dict 再 eval expr），消除了竞态或状态不一致。

识别这些模式后，按正常 `strong` / `possible` 评估，**不标记为 `defensive_fix`**，因为它们是真正的根因修复，只是 PR 作者没有意识到或没有标注为 BugFix。

**关键判断方法**：对这类候选，必须读实际 diff，追问"变更前的代码在用户的场景下会走哪条路径？变更后是否改变了这条路径？"。仅看 `diff_keywords` 和 `ai_summary` 无法做出准确判断。
...
对满足"需要 diff 验证的候选"条件的 PR（见候选评估规则），必须通过以下方式之一读取实际代码变更：

1. `git show <commit_hash> -- '*.java' '*.cpp' '*.h'`（优先，如果本地有该 commit）
2. `gh pr diff <pr_number>`（如果本地没有 commit）
3. `gh pr view <pr_number>` 获取详情后定向读变更文件

读 diff 时的判断框架：

- **变更前**：用户 crash 路径中的代码在变更前会走哪条分支/调用哪个函数？
- **变更后**：同一路径是否被改变（新增检查、切换到不同函数、增加 guard）？
- **覆盖性**：变更后的新路径是否能阻止用户描述的 crash 触发？

如果三个问题的答案都是肯定的，即使 PR 自描述为 Refactor / Enhancement / "no behavior change"，也应判为 `strong`。

Diff 验证总量限制：每次分析最多验证 6 个候选的 diff（含 `/api/agent/pr` 和 `gh` 调用）。优先验证 B 类强命中 + Refactor/Enhancement 类型的候选，因为这些最容易被仅看 metadata 时误判。
...
**企业独有修复标注**：如果证据 PR 只在企业仓库检索到、且不是从开源同步过来的（即企业侧 `synced_from` 为空——这也是 `exclusive` 分类 PR 的常态，因为能被检索到的企业 PR 只有 `exclusive`、从不是 `sync` / `backport` / `conflict_fix`），必须在结论中明确说明"该修复仅企业版（按候选的 `repo`：`cd`=CelerData 或 `ms`=MirrorShip 仓库）包含，开源版未包含"，不要让用户误以为开源版也已修复。
